...
def make_general_histograms(
    df, figsize=(20, 12), xlabel="", group_cols_N=3, columns=None
):
    if columns is None:
        columns = df.columns
    ...
    num_rows = (
        num_plots + group_cols_N - 1
    ) // group_cols_N  # Calculate the number of rows needed for 3 columns
...
def plot_stacked_barchart(
    df, columns, frequency="M", figsize=(20, 6), title="", ylabel="", xlabel=""
):
    """
    Plots a stacked bar chart from the provided dataframe.

    Parameters:
    df (pd.DataFrame): DataFrame with columns 'timestamp' 
    and other numeric columns.
    ...
    ax = dx_resampled.plot(
        kind="bar", stacked=True, figsize=figsize, colormap="rainbow_r"
    )
...
def make_seasonal_decomposition(
        input_data,
        model="multiplicative",
        period=365 * 24):
    decomposition = seasonal_decompose(
        input_data, model=model, period=period
    )
...
def make_categories_pie_chart(df, figsize):
    ...
    for col in text_columns.columns:
        ...
        labels = [
            f"{label}: {pct:.1f}%"
            for label, pct in zip(value_counts.index, value_counts.values)
        ]
...
def count_outliers(df):
    ...
    a new DataFrame with the count of outliers
    for each numerical column.

    An outlier is defined as a data point that is below
    ...
    Returns:
    pd.DataFrame: A DataFrame with the count of
...
def check_remove_duplicates(df, subset_elements, keep_type):
    original_rows = len(df)
    df = df.drop_duplicates(subset_elements, keep=keep_type)
    final_rows = len(df)
